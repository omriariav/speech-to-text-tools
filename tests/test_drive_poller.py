...
class TestDrivePoller(unittest.TestCase):
    def setUp(self):
        ...
        self.enqueue_env = self.root / "enqueue-env.txt"
        self.worker_script = self.root / "fake-worker.sh"
        self.worker_calls = self.root / "worker-calls.txt"
...
printf '%s\\n' "${{AUTO_TRANSCRIBE_SPAWN_WORKER-}}" >> {shell_quote(self.enqueue_env)}
exit {exit_code}
""",
            encoding="utf-8",
        )
    ...
    def write_fake_worker(self, exit_code=0):
        self.worker_script.write_text(
            f"""#!/bin/bash
printf 'worker\\n' >> {shell_quote(self.worker_calls)}
exit {exit_code}
""",
            encoding="utf-8",
        )
    ...
    def test_dry_run_filters_to_mp4_without_touching_ledger(self):
        ...
        self.assertFalse(self.worker_calls.exists())
    ...
    def test_downloads_enqueues_and_writes_ledger_marker(self):
        ...
        self.assertEqual(self.enqueue_env.read_text(encoding="utf-8").strip(), "0")
        self.assertEqual(self.worker_calls.read_text(encoding="utf-8"), "worker\n")
        self.assertTrue(ledger_marker.exists())
        self.assertIn(f"name={file_name}", ledger_marker.read_text(encoding="utf-8"))
        self.assertIn("Draining transcription queue in foreground", self.log_text())
        self.assertIn("Worker drain complete.", self.log_text())
        self.assertIn("Poll complete: 1 new, 0 already handled.", self.log_text())

    def test_drain_queue_disabled_leaves_enqueue_worker_enabled(self):
        file_name = "Team / sync: 2026"
        self.write_drive_list([
            {"id": "video-1", "name": file_name, "mime_type": "video/mp4"},
        ])

        result = self.run_poller(DRIVE_DRAIN_QUEUE="0")

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(self.enqueue_env.read_text(encoding="utf-8").strip(), "")
        self.assertFalse(self.worker_calls.exists())
        self.assertIn("Poll complete: 1 new, 0 already handled.", self.log_text())

    def test_existing_pending_queue_job_triggers_worker_drain(self):
        queue_dir = self.output_dir / ".queue"
        queue_dir.mkdir(parents=True)
        (queue_dir / "stuck.job").write_text("INPUT_PATH=/tmp/file.mp4\nBASE_NAME=stuck\n", encoding="utf-8")
        ...
        self.assertEqual(self.worker_calls.read_text(encoding="utf-8"), "worker\n")
        self.assertIn("Draining transcription queue in foreground", self.log_text())
        self.assertIn("Poll complete: 0 new, 0 already handled.", self.log_text())

    def test_worker_drain_failure_exits_nonzero(self):
        self.write_fake_worker(exit_code=42)
        queue_dir = self.output_dir / ".queue"
        queue_dir.mkdir(parents=True)
        (queue_dir / "stuck.job").write_text("INPUT_PATH=/tmp/file.mp4\nBASE_NAME=stuck\n", encoding="utf-8")
        ...
        self.assertEqual(self.worker_calls.read_text(encoding="utf-8"), "worker\n")
        self.assertIn("ERROR: worker drain failed (exit 42).", self.log_text())
        self.assertFalse((self.output_dir / ".staging" / ".poll.lock").exists())

    def test_live_old_poll_lock_is_not_reclaimed(self):
        lock_dir = self.output_dir / ".staging" / ".poll.lock"
        ...
        self.assertIn("Another poller is running", self.log_text())
